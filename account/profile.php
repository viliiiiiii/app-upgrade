...
function profile_resolve_user_store(): array {
    ...
    foreach ($candidates as $candidate) {
        try {
            $pdo = $candidate['db_key'] ? get_pdo($candidate['db_key']) : get_pdo();
            $columns = $pdo->query('SHOW COLUMNS FROM `users`')->fetchAll(PDO::FETCH_COLUMN);
            $columns = array_map('strval', $columns ?: []);
            if (!in_array($candidate['password_column'], $columns, true)) {
                continue;
            }
            $store = $candidate + ['pdo' => $pdo, 'columns' => $columns];
            return $store;
        } catch (Throwable $e) {
            // try next candidate
        }
    ...
    throw new RuntimeException('Could not locate a compatible users table.');
}
