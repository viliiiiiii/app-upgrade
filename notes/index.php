...
<?php if ($recentNotes): ?>
<section class="card card--surface notes-recent">
  <header class="notes-recent__header">
    <div>
      <h2>Latest activity</h2>
      <p class="muted">Quick glimpse of the most recent notes you can jump back into.</p>
    </div>
    <?php if ($lastUpdatedRelative): ?>
      <span class="notes-recent__timestamp">Updated <?= sanitize($lastUpdatedRelative); ?></span>
    <?php endif; ?>
  </header>
  <ol class="notes-recent__list">
    <?php foreach ($recentNotes as $note):
      $noteId      = (int)($note['id'] ?? 0);
      $noteTitle   = (string)($note['title'] ?? 'Untitled');
      $noteBody    = (string)($note['body'] ?? '');
      $noteShared  = !empty($note['is_shared']) && empty($note['is_owner']);
      $noteDate    = $note['note_date'] ?? $note['created_at'] ?? $note['updated_at'] ?? '';
      $noteRelative= notes__relative_time($note['created_at'] ?? $note['updated_at'] ?? $note['note_date'] ?? null);
      $noteExcerpt = notes__excerpt($noteBody, 120);
    ?>
      <li class="notes-recent__item">
        <a class="notes-recent__link" href="view.php?id=<?= $noteId; ?>">
          <span class="notes-recent__title"><?= sanitize($noteTitle); ?></span>
          <?php if ($noteShared): ?><span class="badge">Shared</span><?php endif; ?>
          <span class="notes-recent__meta">
            <?php if ($noteDate): ?><span><?= sanitize((string)$noteDate); ?></span><?php endif; ?>
            <?php if ($noteRelative): ?><span><?= sanitize($noteRelative); ?></span><?php endif; ?>
          </span>
        </a>
        <?php if ($noteExcerpt !== ''): ?>
          <p class="notes-recent__excerpt"><?= sanitize($noteExcerpt); ?></p>
        <?php endif; ?>
      </li>
    <?php endforeach; ?>
  </ol>
